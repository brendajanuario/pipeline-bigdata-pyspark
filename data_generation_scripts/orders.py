...
def generate_order(items, customers_pd):
    ...
    order = {
        ...
        "order_dt": fake.date_time_between_dates(start_dt, end_dt).strftime("%Y-%m-%d %H:%m:%S"),
        "items": items,
...
def insert_to_s3_bucket(object, record_dt, file_name):
    try:
        ...
        key = f"transactional/orders/record_dt={record_dt}/{file_name}.json"
...
for single_date in daterange(start_date, end_date):
    ...
    for n in range(1, 100):
        ...
        order = generate_order(items, customers_pd)
        orders.append(order)
